...
#include "/mnt/d/works/CMS/UpsilonPbPb2018_v2_Win/UpsilonPbPb2018_v2/commonUtility.h"
#include "/mnt/d/works/CMS/UpsilonPbPb2018_v2_Win/UpsilonPbPb2018_v2/cutsAndBinUpsilonV2.h"
#include "/mnt/d/works/CMS/UpsilonPbPb2018_v2_Win/UpsilonPbPb2018_v2/HiEvtPlaneList.h"
#include "/mnt/d/works/CMS/UpsilonPbPb2018_v2_Win/UpsilonPbPb2018_v2/Style_jaebeom.h"
#include "/mnt/d/works/CMS/UpsilonPbPb2018_v2_Win/UpsilonPbPb2018_v2/tdrstyle.C"
#include "/mnt/d/works/CMS/UpsilonPbPb2018_v2_Win/UpsilonPbPb2018_v2/CMS_lumi_v2mass.C"
...
//}}}
  ...
  TFile* rf = new TFile(Form("/mnt/d/works/CMS/UpsilonPbPb2018_v2_Win/UpsilonPbPb2018_v2/plots/MassV2Hist/Ups_%s_AccW1_EffW1_MuId%s%s_InnTrk%s%s_Trig%s%s.root",kineLabel.Data(),fMuIdSysString.Data(),fMuIdStatString.Data(),fInnTrkSysString.Data(),fInnTrkStatString.Data(),fTrigSysString.Data(),fTrigStatString.Data()),"read");
  ...
  //Read Signal File
  TFile *sfile = new TFile(Form("/mnt/d/works/CMS/UpsilonPbPb2018_v2_Win/UpsilonPbPb2018_v2/MassFit/AllParmFreeFit/fitRes/fitresults_upsilon_DoubleCB_pt%.1f-%.1f_y%.1f-%.1f_muPt3.5_centrality0-180.root",ptLow,ptHigh,yLow,yHigh),"read");
  sfile->cd();
...
//  fitter.Config().ParSettings(12).Fix();
  ...
  TString outputFileName = Form("%s/SimFitResult_TnP_%s_MuId%s%s_InnTrk%s%s_Trig%s%s.txt",fDIR.Data(),kineLabel.Data(),fMuIdSysString.Data(),fMuIdStatString.Data(),fInnTrkSysString.Data(),fInnTrkStatString.Data(),fTrigSysString.Data(),fTrigStatString.Data());
  ofstream outputFile(outputFileName.Data());
  result.Print(outputFile);
  
  cout << "**************************************************************************" << endl;
  cout << " " << endl;
  cout << setw(5) << right << "  " << setw(27) << right << " Inital Value " << setw(15) << right  << " Upper Limit " << setw(15) << right << " Lower Limit " << endl;
  cout << "--------------------------------------------------------------" << endl;
  for(int i=0; i<20; i++){
	  cout << "par[" << i << "]" << "	:	" << setw(15) << right << par0[i] << setw(15) << right << parLimitHigh[i] << setw(15) << right << parLimitLow[i] << endl;
  }
  cout << " " << endl;

  outputFile << "**************************************************************************\n" ;
  outputFile << "\n" ;
  outputFile << setw(5) << right << "  " << setw(27) << right << " Inital Value " << setw(15) << right  << " Upper Limit " << setw(15) << right << " Lower Limit \n" ;
  outputFile << "--------------------------------------------------------------\n" ;
  for(int i=0; i<20; i++){
	  outputFile << "par[" << i << "]" << "	:	" << setw(15) << right << par0[i] << setw(15) << right << parLimitHigh[i] << setw(15) << right << parLimitLow[i] << "\n";
  }
  outputFile << "\n" ;
  outputFile.close();
  
  cout << outputFileName << " is SAVED " << endl;
