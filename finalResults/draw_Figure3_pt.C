...
void draw_Figure3_pt(){
  ...
  gv2[0] = (TGraphErrors*)fIn->Get("gr_point_v2_vs_pt_Cent010");
  gv2[1] = (TGraphErrors*)fIn->Get("gr_point_v2_vs_pt_Cent1030");
  gv2[2] = (TGraphErrors*)fIn->Get("gr_point_v2_vs_pt_Cent3050");
  gv2[3] = (TGraphErrors*)fIn->Get("gr_point_v2_vs_pt_Cent5090");
  gsys[0] = (TGraphErrors*)fIn->Get("gr_sys_v2_vs_pt_Cent010");
  gsys[1] = (TGraphErrors*)fIn->Get("gr_sys_v2_vs_pt_Cent1030");
  gsys[2] = (TGraphErrors*)fIn->Get("gr_sys_v2_vs_pt_Cent3050");
  gsys[3] = (TGraphErrors*)fIn->Get("gr_sys_v2_vs_pt_Cent5090");

  final_v2 = new TH1D("Final_v2",";p_{T}^{#Upsilon(1S)} (GeV/c);v_{2}^{Sig}",nBin,0,3);
  for(int ist = 0; ist < nCentBin; ist++){
      ...
      for (int is=0; is<nBin; is++){
          ...
          gv2[ist] -> GetPoint(is,pxtmp,pytmp);
          eytmp = gv2[ist]->GetErrorY(is);
      ...
      double eXdiv = 6.0; // reduce x sys size (1/eXdiv)
      ...
      for(int ib=0;ib<final_v2->GetNbinsX();ib++){
          ...
          gsys[ist]->GetPoint(ib,sysX,sysY);
          sysY_Error = gsys[ist]->GetErrorY(ib);
      ...
      if(ist == 0) globtex->DrawLatex(0.23, sz_init-sz_step, Form("Cent. 0-10%s",perc.Data()));
      if(ist == 1) globtex->DrawLatex(0.23, sz_init-sz_step, Form("Cent. 10-30%s",perc.Data()));
      if(ist == 2) globtex->DrawLatex(0.23, sz_init-sz_step, Form("Cent. 30-50%s",perc.Data()));
      if(ist == 3) globtex->DrawLatex(0.23, sz_init-sz_step, Form("Cent. 50-90%s",perc.Data()));
      ...
      c1->Update();
      c1->SaveAs(Form("v2Sig_pt_Cent_%d.pdf",ist));
  }
