...
*/  
  save->cd();
  hNtrk = new TH1D("Ntrk","Ntrk",1001,0,3000);
  int npt = nptbins;
  for(int iorder = 1; iorder<=7; iorder++) {
    qxtrk[iorder-1] = new TH2D(Form("qxtrk%d",iorder),Form("qxtrk%d",iorder),npt,ptbins, netabinsDefault, etabinsDefault);
    qytrk[iorder-1] = new TH2D(Form("qytrk%d",iorder),Form("qytrk%d",iorder),npt,ptbins, netabinsDefault, etabinsDefault);
  ...
  qcnt =  new TH2D("qcnt", "qcnt",npt,ptbins, netabinsDefault, etabinsDefault);
  ...
  hTemplate->Reset();
  hcent = new TH1D("cent","cent",220,-10,110);
  hvtx = new TH1D("vtx","vtx",600,-30,30);
  hvtxRaw = new TH1D("vtxRaw","vtxRaw",600,-30,30);
  hb = new TH1D("b","b",10000,0,10000);
  hRP = new TH1D("rp","rp",1000,-4,4);
  hcentbins = new TH1D("centbins","centbins",201,0,200);
  hcentres = new TH1D("centres","centres",ncentbins,centbins);
...
//    ressubdir = resdir.mkdir(Form("%d_%d",(int)centbins[i],(int)centbins[i+1]));
    for(int iorder = 1; iorder<=7; iorder++) {
      res[iorder-1][i] = new TH2D(Form("res%d",iorder),Form("res%d",iorder),50,0,50,50,0,50);
      resw[iorder-1][i] = new TH2D(Form("resw%d",iorder),Form("resw%d",iorder),50,0,50,50,0,50);
      resep[iorder-1][i] = new TH2D(Form("resep%d",iorder),Form("resep%d",iorder),50,0,50,50,0,50);
      rescnt[iorder-1][i] = new TH2D(Form("rescnt%d",iorder),Form("rescnt%d",iorder),50,0,50,50,0,50);
...
//    subdir = hardir.mkdir(Form("%d_%d",(int)centbins[i],(int)centbins[i+1]));
    ptav[i] = new TH2D("ptav","ptav",npt,ptbins, netabinsDefault, etabinsDefault);
    ptcnt[i] = new TH2D("ptcnt","ptcnt",npt,ptbins, netabinsDefault, etabinsDefault);
    ptav[i]->Sumw2();
    ptcnt[i]->Sumw2();
...
//      TFileDirectory andir = subdir.mkdir(AnalNames[ian].data());
      ...
      qanal[ian].qA[i][0] = new TH2D("qA","qA",npt,ptbins, netabinsDefault, etabinsDefault);
      qanal[ian].qB[i][0] = new TH2D("qB","qB",npt,ptbins, netabinsDefault, etabinsDefault);
      qanal[ian].wnA[i][0] = new TH2D("wnA","wnA",npt,ptbins, netabinsDefault, etabinsDefault);
      qanal[ian].wnB[i][0] = new TH2D("wnB","wnB",npt,ptbins, netabinsDefault, etabinsDefault);
      ...
      qanal[ian].thA[i][0]  = new TH1D("thA","thA",160,-4,4);
      qanal[ian].thB[i][0]  = new TH1D("thB","thB",160,-4,4);
      qanal[ian].thC[i][0]  = new TH1D("thC","thC",160,-4,4);
      qanal[ian].thA2[i][0] = new TH1D("thA2","thA2",160,-4,4);
      qanal[ian].thB2[i][0] = new TH1D("thB2","thB2",160,-4,4);
      qanal[ian].thC2[i][0] = new TH1D("thC2","thC2",160,-4,4);
      qanal[ian].thn[i][0]  = new TH1D("thn","thn",160,-4,4);

      qanal[ian].qBA[i][0] = new TH1D("qBA","qBA",1,0,1);
      qanal[ian].qCA[i][0] = new TH1D("qCA","qCA",1,0,1);
      qanal[ian].qCB[i][0] = new TH1D("qCB","qCB",1,0,1);
      qanal[ian].qBAcnt[i][0] = new TH1D("qBAcnt","qBAcnt",1,0,1);
      qanal[ian].qCAcnt[i][0] = new TH1D("qCAcnt","qCAcnt",1,0,1);
      qanal[ian].qCBcnt[i][0] = new TH1D("qCBcnt","qCBcnt",1,0,1);
 

      qanal[ian].qBA2[i][0] = new TH1D("qBA2","qBA2",1,0,1);
      qanal[ian].qCA2[i][0] = new TH1D("qCA2","qCA2",1,0,1);
      qanal[ian].qCB2[i][0] = new TH1D("qCB2","qCB2",1,0,1);
      qanal[ian].qBAcnt2[i][0] = new TH1D("qBAcnt2","qBAcnt2",1,0,1);
      qanal[ian].qCAcnt2[i][0] = new TH1D("qCAcnt2","qCAcnt2",1,0,1);
      qanal[ian].qCBcnt2[i][0] = new TH1D("qCBcnt2","qCBcnt2",1,0,1);
     
//      TFileDirectory subev = andir.mkdir("SubEvents");
      for(int j = 1; j<=10; j++) {	  
	qanal[ian].qA[i][j] = new TH2D(Form("qA_%d",j),Form("qA_%d",j),npt,ptbins, netabinsDefault, etabinsDefault); 
	qanal[ian].qB[i][j] = new TH2D(Form("qB_%d",j),Form("qB_%d",j),npt,ptbins, netabinsDefault, etabinsDefault);
	qanal[ian].wnA[i][j] = new TH2D(Form("wnA_%d",j),Form("wnA_%d",j),npt,ptbins, netabinsDefault, etabinsDefault);
	qanal[ian].wnB[i][j] = new TH2D(Form("wnB_%d",j),Form("wnB_%d",j),npt,ptbins, netabinsDefault, etabinsDefault);
	...
	qanal[ian].thA[i][j]  = new TH1D(Form("thA_%d",j),Form("thA_%d",j),160,-4,4);
	qanal[ian].thB[i][j]  = new TH1D(Form("thB_%d",j),Form("thB_%d",j),160,-4,4);
	qanal[ian].thC[i][j]  = new TH1D(Form("thC_%d",j),Form("thC_%d",j),160,-4,4);
	qanal[ian].thA2[i][j]  = new TH1D(Form("thA2_%d",j),Form("thA2_%d",j),160,-4,4);
	qanal[ian].thB2[i][j]  = new TH1D(Form("thB2_%d",j),Form("thB2_%d",j),160,-4,4);
	qanal[ian].thC2[i][j]  = new TH1D(Form("thC2_%d",j),Form("thC2_%d",j),160,-4,4);
	qanal[ian].thn[i][j]  = new TH1D(Form("thn_%d",j),Form("thn_%d",j),160,-4,4);
	
	qanal[ian].qBA[i][j] = new TH1D(Form("qBA_%d",j),Form("qBA_%d",j),1,0,1);
	qanal[ian].qCA[i][j] = new TH1D(Form("qCA_%d",j),Form("qCA_%d",j),1,0,1);      
	qanal[ian].qCB[i][j] = new TH1D(Form("qCB_%d",j),Form("qCB_%d",j),1,0,1);
	
	qanal[ian].qBAcnt[i][j] = new TH1D(Form("qBAcnt_%d",j),Form("qBAcnt_%d",j),1,0,1);
	qanal[ian].qCAcnt[i][j] = new TH1D(Form("qCAcnt_%d",j),Form("qCAcnt_%d",j),1,0,1);      
	qanal[ian].qCBcnt[i][j] = new TH1D(Form("qCBcnt_%d",j),Form("qCBcnt_%d",j),1,0,1);
	
	qanal[ian].qBA2[i][j] = new TH1D(Form("qBA2_%d",j),Form("qBA2_%d",j),1,0,1);
	qanal[ian].qCA2[i][j] = new TH1D(Form("qCA2_%d",j),Form("qCA2_%d",j),1,0,1);      
	qanal[ian].qCB2[i][j] = new TH1D(Form("qCB2_%d",j),Form("qCB2_%d",j),1,0,1);
	
	qanal[ian].qBAcnt2[i][j] = new TH1D(Form("qBAcnt2_%d",j),Form("qBAcnt2_%d",j),1,0,1);
	qanal[ian].qCAcnt2[i][j] = new TH1D(Form("qCAcnt2_%d",j),Form("qCAcnt2_%d",j),1,0,1);      
	qanal[ian].qCBcnt2[i][j] = new TH1D(Form("qCBcnt2_%d",j),Form("qCBcnt2_%d",j),1,0,1);
...
VNAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if (privtxs.isValid()){
    ...
    hPileUp->Fill(nPV);
     ...
     std::cout<<"VNAnalyzer ERROR: privtxs is NULL or not isValid ! Return now"<<std::endl; return;
     centval = -100;
     tree->Fill();
     std::cout << "*** centval : -100, run : " << runNb << " , event : " << eventNb << " , LS : " << lumiSection << std::endl;
  }
...
//  if(ntrkval<=0) {std::cout << "ntrkval<=0: ntrakval: " << ntrkval<<std::endl;return;}
  if(ntrkval<=0) {
  		if (makeTree_) {
	     std::cout << "*** centval : -200, run : " << runNb << " , event : " << eventNb << " , LS : " << lumiSection << std::endl;
 		std::cout << "ntrkval<=0: ntrakval: " << ntrkval<<std::endl;
			centval = -200;
			tree->Fill();
		}
			return;
  ...
  if(!inputPlanes_.isValid()){
    cout << "Error! Can't get hiEvtPlaneFlat product!" << endl;
  		if (makeTree_) {
	     std::cout << "*** centval : -300, run : " << runNb << " , event : " << eventNb << " , LS : " << lumiSection << std::endl;
 		std::cout << "But, we will fill a null tree...." <<std::endl;
			centval = -300;
			tree->Fill();
		}
  ...
  for (EvtPlaneCollection::const_iterator rp = inputPlanes_->begin();rp !=inputPlanes_->end(); rp++) {
    if(indx != rp->indx() ) {
      cout<<"EP inconsistency found. Abort."<<endl;
  		if (makeTree_) {
	     std::cout << "*** centval : -400, run : " << runNb << " , event : " << eventNb << " , LS : " << lumiSection << std::endl;
 		std::cout << "But, we will fill a null tree...." <<std::endl;
			centval = -400;
			tree->Fill();
		}
