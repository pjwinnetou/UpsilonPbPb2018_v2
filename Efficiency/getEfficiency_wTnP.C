...
void getEfficiency_wTnP(
  ...
  for(int iev=0; iev<nevt ; ++iev)
  {
    ...
    for (Int_t irqq=0; irqq<Reco_QQ_size; ++irqq) 
    {
      ...
      if(Centrality < 20) hptLowestC_reco -> Fill(JP_Reco->Pt(), weight * tnp_weight);
      else if(Centrality > 20 && Centrality < 60) hptLowC_reco -> Fill(JP_Reco->Pt(), weight * tnp_weight);
      else if(Centrality > 60 && Centrality < 100) hptMidC_reco -> Fill(JP_Reco->Pt(), weight * tnp_weight);
